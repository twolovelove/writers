...
하루에 1,000자씩, 매일 조금씩 글쓰기 습관을 만들어주는 웹앱입니다. 매일 자동으로 추천되는 글감 하나로 원고지처럼 편안하게 글을 쓰고, 완료하면 규칙 기반 첨삭 노트를 받아볼 수 있습니다.

## 주요 기능

- **오늘의 추천 글감**: 에세이 / 상상·소설 / 자기계발 / 비평 / 자유주제 5개 카테고리 중 그날 하나가 날짜 기준으로 결정론적으로 정해짐(유저가 직접 고르지 않음, 워들처럼 "그날은 그 글감 하나"). 홈 화면엔 "글쓰기 시작" 버튼만 있고, 실제 글감 내용은 에디터에 들어가야 공개됨
...
- **아카이브**: 완료한 글만 달력·목록으로 모아 읽기 전용으로 다시 볼 수 있음(하루 1회 완료 잠금 때문에 다시 이어 쓸 수 없는 미완성 초고는 노출하지 않음)
- **모음집**: 완료한 글만 책처럼 엮어 월별로 보거나 목차와 함께 인쇄·PDF로 저장
...
`operations/로드맵.md`의 "배포 시점 직전 (P0.5)" 체크리스트를 참고하세요. Edge Function 배포 후에는
...
운영·유지보수 관점의 우선순위와 아직 열려있는 작업은 [`operations/`](./operations/README.md) 폴더를 참고하세요.
